...
function renderProfileHeader() {
    ...
    if (viewedUser?.id) {
        ...
        if (profileIdentityLink) {
            ...
            profileIdentityLink.setAttribute(
                "aria-label",
...
function openEditForm() {
    ...
    if (avatarPreview) {
        avatarPreview.src = getAvatarSrc(
            currentUser,
            "../assets/default-avatar.svg",
        );
    }
...
async function handleSave() {
    ...
    const changedUsername = changeUsernameInput
...
function validateProfileInput(username) {
    if (!username) {
        return {
            ok: false,
            field: "username",
            message: "Cannot have an empty username",
        };
    }
