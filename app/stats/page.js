...
function DisplayBarChartFunc({ DataToHandle }) {
    ...
    const MaximumCount = Math.max(
        ...DataToHandle.map((DataEntry) => DataEntry.count),
        1,
    );
    return (
        <div className="stats-bar-chart" aria-label="Posts per day chart">
            {DataToHandle.map((Entry, indexation) => {
                const PercentageRounded = Math.round(
                    (Entry.count / MaximumCount) * 100,
                );
                const DateLabel =
                ...
                return (
                    ...
                    >
                        <div
                            ...
                            style={{
                                height: `${Math.max(PercentageRounded, 2)}%`,
                            }}
                            aria-label={`${Entry.count} posts`}
                        />
                        {indexation % 7 === 0 && (
                            <span className="stats-bar-label">{DateLabel}</span>
                        )}
                    </div>
                );
            })}
...
function WordCloudSizeRank({ words }) {
    ...
    const MaximumCounter = Math.max(
        ...words.map((wordentry) => wordentry.count),
        1,
    );
    return (
        <div className="stats-word-cloud">
            {words.map((wordentry) => {
                ...
                return (
                    <span
                        key={wordentry.word}
                        className={`stats-word stats-word-${size}`}
                    >
                        {wordentry.word}
                    </span>
                );
            ...
            <div className="content-column stats-page page-enter">
                ...
                <section className="stats-section">
                    ...
                    <div className="stats-totals-grid">
                        <StatCardNumber
                            PassedLabel="Users"
                            GivenValue={TotalsData.users.toLocaleString()}
                        />
                        <StatCardNumber
                            PassedLabel="Posts"
                            GivenValue={TotalsData.posts.toLocaleString()}
                        />
                        ...
                        <StatCardNumber
                            PassedLabel="Likes"
                            GivenValue={TotalsData.likes.toLocaleString()}
                        />
                ...
                >
                    ...
                    >
                        <p className="stats-section-title">
                            Posts per day in the last 30 days
                        </p>
                    ...
                    >
                        ...
                        <div className="stats-ranked-list">
                            {RankedTopPosts.map((EachPosting, Index) => (
                                <div
                                    key={EachPosting.id}
                                    className="stats-ranked-item"
                                >
                                    <span
                                        className={`stats-rank-num${Index < 3 ? " top3" : ""}`}
                                    >
                                        {Index + 1}
                                    </span>
                                    <div className="stats-ranked-info">
                                        <p className="stats-ranked-name">
                                            {" "}
                                            @{EachPosting.author.username}
                                        </p>
                                ...
                                >
                                    ...
                                    {EachUser.profilePicture
                                        ? <img
                                          ...
                                          />
                                        : <div
                                              className="stats-avatar-placeholder"
                                              aria-hidden="true"
                                          >
                ...
                <section className="stats-section">
                    <p className="stats-section-title">
                        Most Active in The Last 3 months
                    </p>
                    <div className="stats-ranked-list">
